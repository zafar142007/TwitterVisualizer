...
public class TwitterThreadProvider extends Thread {
	...
	public TwitterThreadProvider(String lat, String longi,
	...
	public String calculateBoundingBox() {
		...
		try {
			...
			result.append(convertToDegrees(convertToRadians(longi)
					- (RADIUS / EARTH_RADIUS)));
			result.append(",");
			result.append(convertToDegrees(convertToRadians(lat)
					- (RADIUS / EARTH_RADIUS)));
	...
	public void run() {
		try {
			...
			request.addBodyParameter("locations", searchBoundingBox); // Set
																		// keywords
																		// you'd
																		// like
																		// to
				...
				synchronized (tweets) {
					if (line != "") {
						...
						if (tweets.size() == MAX_NUMBER_OF_TWEETS) {
							tweets.remove(0);// flush out from the front of the
												// queue
						}
					}
				}
			...
			reader.close();
			log.debug("Closing this thread: latitude " + latitude
					+ "  longitude " + longitude);
		} catch (Exception e) {
			e.printStackTrace();
		}
